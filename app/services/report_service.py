...
class ProductionReportGenerator:
    ...
    def _setup_custom_styles(self):
        ...
        self.styles.add(
            ParagraphStyle(
                name='MetricLabel', parent=self.styles['Normal'], fontSize=9, textColor=colors.HexColor('#666666')
            )
        )
    ...
    def generate_production_report(
        self, start_date: str = None, end_date: str = None, po_numbers: list[str] = None, tablet_type_id: int = None
    ) -> bytes:
        """
        ...
        try:
            ...
            doc = SimpleDocTemplate(
                buffer, pagesize=letter, rightMargin=50, leftMargin=50, topMargin=30, bottomMargin=25
            )
            ...
            story.append(
                Paragraph(f"Generated on {now_et.strftime('%B %d, %Y at %I:%M %p')} {tz_abbr}", self.styles['Normal'])
            )
            ...
            raise Exception(f"Failed to generate production report: {str(e)}") from e
    ...
    def _get_report_data(
        ...
        try:
            ...
            if report_data['summary']['total_ordered'] > 0:
                ...
                report_data['summary']['efficiency_rate'] = (
                    (report_data['summary']['total_produced'] / total_processed * 100) if total_processed > 0 else 0
                )

            # Get product breakdown
            report_data['summary']['product_breakdown'] = self._get_product_breakdown(
                conn, start_date, end_date, po_numbers, tablet_type_id
            )
        ...
        try:
            ...
            return {
                **po_dict,
                ...
                'pack_time_days': self._calculate_pack_time(
                    {'po': po_dict, 'shipment': shipment_dict, 'submissions': submissions_list}
                ),
            }
        except Exception:
    ...
    def _calculate_production_breakdown(self, submissions: list[dict]) -> dict:
        ...
        for sub in submissions:
            ...
            displays_tablets = (
                (sub['displays_made'] or 0) * (sub['packages_per_display'] or 0) * (sub['tablets_per_package'] or 0)
            )
            ...
            if employee not in breakdown['by_employee']:
                breakdown['by_employee'][employee] = {'submissions': 0, 'displays': 0, 'total_tablets': 0}
    ...
    def _calculate_pack_time(self, po_data: dict) -> float | None:
    ...
    def _create_executive_summary(self, report_data: dict) -> list:
        ...
        summary_data = [
            ...
            [
                'Average Pack Time',
                f"{summary['average_pack_time']:.1f} days" if summary['average_pack_time'] else "N/A",
            ],
        ]
        ...
        if summary.get('product_breakdown'):
            ...
            remaining_header = Paragraph(
                "<font color='white'>Remaining</font><br/><font size=7 color='white'>Ordered - (Produced + Damaged)</font>",
                self.styles['Normal'],
            )
            ...
            for product in summary['product_breakdown']:
                ...
                product_data.append(
                    [product['product_name'], f"{ordered:,}", f"{produced:,}", f"{damaged:,}", f"{remaining:,}"]
                )
    ...
    def _create_po_detailed_report(self, po_data: dict) -> list:
        ...
        if po_data.get('lines'):
            ...
            for line in po_data['lines']:
                remaining = (
                    (line.get('quantity_ordered', 0) or 0)
                    - (line.get('good_count', 0) or 0)
                    - (line.get('damaged_count', 0) or 0)
                )
            ...
            line_items_table = Table(
                line_items_data, colWidths=[2.5 * inch, 0.8 * inch, 0.9 * inch, 0.9 * inch, 0.9 * inch, 0.9 * inch]
            )
            ...
            if breakdown['by_employee']:
                ...
                for employee, data in breakdown['by_employee'].items():
                    employee_data.append(
                        [employee, str(data['submissions']), str(data['displays']), f"{data['total_tablets']:,}"]
                    )
    ...
    def generate_vendor_report(
        ...
        try:
            ...
            if summary.get('product_breakdown') and len(summary['product_breakdown']) > 0:
                ...
                product_table = Table(
                    product_data, colWidths=[2.5 * inch, 1.1 * inch, 1.1 * inch, 1.1 * inch, 1.2 * inch]
                )
                product_table.setStyle(
                    TableStyle(
                        [
                ...
                story.append(
                    Paragraph("No production data available for the selected criteria.", self.styles['Normal'])
                )
                story.append(Spacer(1, 10))
            ...
            raise Exception(f"Failed to generate vendor report: {str(e)}") from e
    ...
    def generate_receive_report(self, receive_id: int) -> bytes:
        ...
        try:
            ...
            if receive.get('receive_name'):
                ...
                ''',
                    (receive['po_id'], receive.get('received_date'), receive.get('received_date'), receive['id']),
                ).fetchone()
