...
def find_matching_bags(
    conn: sqlite3.Connection, submission_dict: dict[str, Any], exclude_closed_bags: bool = False
) -> list[dict[str, Any]]:
    """
    ...
    if not tablet_type_id and submission_dict.get('inventory_item_id'):
        ...
        ''',
            (submission_dict.get('inventory_item_id'),),
        ).fetchone()
        if tt_row:
...
def build_receive_name(bag: dict[str, Any], conn: sqlite3.Connection) -> str:
    """
        ...
        receive_number_result = conn.execute(
            ...
            AND (r2.received_date < (SELECT received_date FROM receiving WHERE id = ?)
                 OR (r2.received_date = (SELECT received_date FROM receiving WHERE id = ?)
                     AND r2.id < ?))
        ''',
            (po_id, receive_id, receive_id, receive_id),
        ).fetchone()
