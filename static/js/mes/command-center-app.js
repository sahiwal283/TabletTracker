(function () {
  ...
  function renderDPP115BlisterMachine(status) {
    return machineSvgWrap(html`<circle cx="26" cy="58" r="13" className="part" />
      <rect x="45" y="49" width="58" height="18" className="part" /><rect x="113" y="41" width="36" height="34" className="part" />
      <rect x="154" y="64" width="46" height="9" className="part" />
      <g className="pill-grid">${Array.from({ length: 5 }).map(function (_, i) { return html`<circle key=${i} cx=${50 + i * 10} cy="58" r="2.6" className="pill"/>`; })}</g>`, status);
  }
  function renderHeatPressMachine(status) {
    return machineSvgWrap(html`<rect x="40" y="28" width="140" height="18" className="part" /><rect x="35" y="70" width="150" height="16" className="part" />
      <line x1="52" y1="46" x2="52" y2="70" className="line" /><line x1="168" y1="46" x2="168" y2="70" className="line" />
      <path d="M90 52 l8 10 l8-10" className="arrow" /><path d="M120 52 l8 10 l8-10" className="arrow" />`, status);
  }
  function renderStickeringMachine(status) {
    return machineSvgWrap(html`<circle cx="42" cy="42" r="14" className="part" /><rect x="58" y="35" width="56" height="14" className="part" />
      <rect x="120" y="34" width="18" height="28" className="part" /><rect x="142" y="68" width="54" height="9" className="part" />
      <path d="M58 42 h-10" className="line" />`, status);
  }
  function renderBottleSealingMachine(status) {
    return machineSvgWrap(html`<path d="M58 80 h24 l6-28 h-8 v-11 h-20 v11 h-8 z" className="part" /><rect x="98" y="34" width="20" height="26" className="part" />
      <ellipse cx="156" cy="78" rx="32" ry="12" className="part" /><rect x="130" y="72" width="52" height="8" className="part" />`, status);
  }
  function renderPackagingStation(status) {
    return machineSvgWrap(html`<rect x="36" y="68" width="58" height="24" className="part" /><rect x="104" y="60" width="38" height="32" className="part" />
      <rect x="146" y="52" width="40" height="40" className="part" /><line x1="36" y1="68" x2="188" y2="68" className="line" />`, status);
  }
  ...
  function KpiCard(props) {
    return html`<div className="mes-kpi-card"><div className="label">${props.label}</div><div className="value">${props.value}</div>
      ${props.spark && props.spark.length > 1 ? html`<svg className="spark" viewBox="0 0 100 18">${props.spark.map(function (v, i) {
      ...
      })}</svg>` : html`<div className="spark-empty">No data</div>`}
    </div>`;
  }
  ...
  function ProcessLane(props) {
    return html`<div className=${"mes-lane " + (props.dimmed ? "dim" : "") }>
      ...
      <div className="lane-track">${props.nodes.map(function (n, idx) {
        return html`<div key=${idx} className="lane-node"><div className="n-top"><strong>${n.name}</strong><em>${n.wip > 0 ? "Active" : "Idle"}</em></div>
          <div className="n-meta">WIP ${n.wip || 0} · bags ${n.bags || 0}</div><div className="n-meta">Oldest dwell ${n.dwell || "N/A"}</div></div>`;
      })}</div>
    </div>`;
  }
  ...
  function MachineCard(props) {
    ...
    return html`<article className=${"mes-machine-card " + statusClass(status)}>
      <header><h3>${m.shortLabel} · ${m.label}</h3><span className="badge">${status === "LIVE_QR" ? "LIVE QR" : status === "NO_ACTIVITY_TODAY" ? "NO ACTIVITY TODAY" : status}</span></header>
      <div className="svg-wrap">${renderMachineIllustration(m.slot, status)}</div>
      <dl>
        <dt>Station ID</dt><dd>${m.stationId != null ? m.stationId : "N/A"}</dd>
  ...
  function TracePanel(props) {
    ...
    return html`<section className="mes-panel trace" id="lot-trace-panel">
      <div className="panel-title">Live Bag Genealogy / Lot Trace</div><label for="trace-bag">Trace bag ID</label>
      ...
      <div className="trace-lines">${(geo.traceLines || []).map(function (r, idx) {
        return html`<div key=${idx} className=${"trace-row " + (r.pending ? "pending" : "done") }>
          <span>${r.label}</span><span>${r.atMs ? new Date(r.atMs).toLocaleTimeString() : "—"}</span><span>${r.machineLabel || "—"}</span><span>${r.operatorLabel || "—"}</span>
          <span>${r.counterReading != null ? r.counterReading : "—"}</span><span>${r.dwellFromPrevMinutes != null ? r.dwellFromPrevMinutes.toFixed(1) + "m" : "—"}</span><span>${r.statusBadge || (r.pending ? "Pending" : "Done")}</span>
        </div>`;
      })}</div>
    </section>`;
  }
  ...
  function StagingPanel(props) {
    ...
    return html`<section className="mes-panel">
      ...
      <table className="mini-table"><thead><tr><th>Order</th><th>Bag</th><th>Idle</th><th>Entered</th><th>Last station</th><th>Last event</th></tr></thead>
      <tbody>
        ${rows.length ? rows.map(function (r, i) { return html`<tr key=${i}><td>#${i + 1}</td><td>${r.bagId}</td><td>${r.idleMinutes.toFixed(1)}m</td><td>${shortClock(r.enteredAtMs)}</td><td>${r.lastStationLabel || "—"}</td><td>${r.lastEventType}</td></tr>`; })
  ...
  function MachineSettingsPanel(props) {
    ...
    return html`<section className="mes-panel">
      ...
      <table className="mini-table"><thead><tr><th>Slot</th><th>Label</th><th>Station ID</th><th>Kind</th><th>Status</th></tr></thead>
      ...
      <div className="muted">Edit station mapping in workflow QR admin.</div>
    </section>`;
  }
  ...
  function AlertsPanels(props) {
    ...
    return html`<section className="mes-panel">
      ...
      <div className="feed-list">${alerts.length ? alerts.map(function (a, i) {
    ...
    <section className="mes-panel">
      ...
      <div className="feed-list">${activity.length ? activity.map(function (a, i) {
  ...
  function App(props) {
    ...
    useEffect(function () {
      ...
      if (n && n.textContent) { try { setSnap(JSON.parse(n.textContent)); } catch (e) {} }
      function load() {
    ...
    var machineDefs = [
      ...
      { slot: 2, shortLabel: "M2", label: "Heat Press / Card Sealing", stationId: inp.slots && inp.slots[1] ? inp.slots[1].stationId : null },
      { slot: 3, shortLabel: "M3", label: "Heat Press / Card Sealing", stationId: inp.slots && inp.slots[2] ? inp.slots[2].stationId : null },
    ...
    var machineCards = machineDefs.map(function (d) {
      ...
      return Object.assign({}, d, metrics, { integrationStatus: status, stationId: stationId, currentBagId: metrics.currentBagId });
    });

    var kpiBy = {};
    (derived.kpis || []).forEach(function (k) { kpiBy[k.id] = k; });

    var alertsOnly = (mes.alerts || []).filter(function (a) { var s = String(a.severity || "").toLowerCase(); return s === "alert" || s === "warn"; });
    var activityFeed = ((mes.timeline || []).map(function (t) { return { atMs: t.at_ms, event: [t.event, t.machine, t.bag_id].filter(Boolean).join(" · ") }; }));

    return html`<div className="mes-app">
      <aside className="mes-aside">${readNavBoot().map(function (n, i) { return html`<a key=${i} href=${n.href} className=${"mes-nav-a " + (String(n.href).indexOf("ops-tv") >= 0 ? "mes-active" : "")}>${n.label}</a>`; })}</aside>
      <main className="mes-main">
        <header className="mes-header"><h1>Pill Packing Command Center</h1><div>${new Date().toLocaleString()}</div></header>
        ...
        <section className="band band-map">
          <div className="map-lanes">
            <${ProcessLane} title="Blister SKU Flow" nodes=${[{ name: "Raw Material Receipt", wip: 0, bags: 0, dwell: "N/A" },{ name: "Machine 1 DPP115", wip: kpiBy.cycles ? kpiBy.cycles.value : 0, bags: kpiBy.bags ? kpiBy.bags.value : 0, dwell: "Live" },{ name: "Post-Blister Staging", wip: derived.stagingBags.length, bags: derived.stagingBags.length, dwell: derived.stagingBags[0] ? derived.stagingBags[0].idleMinutes.toFixed(1) + "m" : "N/A" },{ name: "Machine 2 OR Machine 3 Heat Seal", wip: 0, bags: 0, dwell: "N/A" },{ name: "Post-Seal Staging", wip: 0, bags: 0, dwell: "N/A" },{ name: "Packaging", wip: 0, bags: 0, dwell: "N/A" },{ name: "Finished Goods", wip: 0, bags: 0, dwell: "N/A" }]} />
            <${ProcessLane} title="Bottle SKU Flow" subtitle="Bottle line not integrated yet" dimmed=${true} nodes=${[{ name: "Raw Material", wip: 0, bags: 0, dwell: "N/A" },{ name: "Machine 5 Bottle Sealing", wip: 0, bags: 0, dwell: "N/A" },{ name: "Bottle QA Hold", wip: 0, bags: 0, dwell: "N/A" },{ name: "Finished Goods", wip: 0, bags: 0, dwell: "N/A" }]} />
            <${ProcessLane} title="Card / Stickering SKU Flow" nodes=${[{ name: "Raw Material", wip: 0, bags: 0, dwell: "N/A" },{ name: "Machine 4 Stickering", wip: 0, bags: 0, dwell: "N/A" },{ name: "Packaging", wip: 0, bags: 0, dwell: "N/A" },{ name: "Finished Goods", wip: 0, bags: 0, dwell: "N/A" }]} />
          </div>
          <aside className="alert-rail"><h4>Active alerts</h4>${alertsOnly.length ? alertsOnly.slice(0, 8).map(function (a, i) { return html`<div key=${i} className="alert-row">${a.message}</div>`; }) : html`<div className="alert-row">No active alerts</div>`}</aside>
        </section>

        <section className="band band-machines">${machineCards.map(function (m, i) { return html`<${MachineCard} key=${i} machine=${m} />`; })}</section>
