...
async function viewPODetailsModal(poId, poNumber) {
    ...
    try {
        ...
        const parentBtn = data.parent_po
            ? `<button data-view-parent-po-id="${data.parent_po.id}" data-view-parent-po-number="${escapeHtml(data.parent_po.po_number)}" class="text-sm bg-white bg-opacity-20 hover:bg-opacity-30 px-3 py-1 rounded transition-all">← Parent PO: ${escapeHtml(data.parent_po.po_number)}</button>`
            : '';
        const oversBtn = data.overs_po
            ? `<button data-view-overs-po-id="${data.overs_po.id}" data-view-overs-po-number="${escapeHtml(data.overs_po.po_number)}" class="text-sm bg-white bg-opacity-20 hover:bg-opacity-30 px-3 py-1 rounded transition-all ml-2">Overs PO: ${escapeHtml(data.overs_po.po_number)} →</button>`
            : '';
        ...
        const modalHTML =
            '<div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" data-close-po-details-overlay>' +
            '<div class="bg-white rounded-lg shadow-xl max-w-4xl w-full mx-4 max-h-[90vh] overflow-y-auto" role="dialog" aria-modal="true" aria-label="PO details">' +
            ...
            '<button data-close-po-details-modal class="text-white hover:text-gray-200 text-2xl font-bold">×</button></div></div>' +
            ...
            '<div class="flex justify-end"><button data-close-po-details-modal class="btn-secondary">Close</button></div></div></div></div>';
...
async function viewReceiveDetailsModal(receiveId, receiveName) {
    ...
    try {
        ...
        const modalHTML =
            '<div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" data-close-receive-details-overlay>' +
            '<div class="bg-white rounded-lg shadow-xl max-w-4xl w-full mx-4 max-h-[90vh] overflow-y-auto" role="dialog" aria-modal="true" aria-label="Receive details">' +
            ...
            '<button data-close-receive-details-modal class="text-white hover:text-gray-200 text-2xl font-bold">×</button></div></div>' +
            '<div class="p-6"><h3 class="text-lg font-semibold mb-4">Boxes and Bags</h3><div class="space-y-4">' + boxesHtml + '</div>' +
            '<div class="flex justify-end mt-6"><button data-close-receive-details-modal class="btn-secondary">Close</button></div></div></div></div>';
        const modalDiv = document.createElement('div');
...
document.addEventListener('click', function (event) {
    const poOverlay = event.target.closest('[data-close-po-details-overlay]');
    if (poOverlay && event.target === poOverlay) {
        closePODetailsModal(event);
        return;
    }
    if (event.target.closest('[data-close-po-details-modal]')) {
        closePODetailsModal();
        return;
    }
    const parentBtn = event.target.closest('[data-view-parent-po-id]');
    if (parentBtn) {
        closePODetailsModal();
        setTimeout(() => viewPODetailsModal(parseInt(parentBtn.dataset.viewParentPoId, 10), parentBtn.dataset.viewParentPoNumber || ''), 100);
        return;
    }
    const oversBtn = event.target.closest('[data-view-overs-po-id]');
    if (oversBtn) {
        closePODetailsModal();
        setTimeout(() => viewPODetailsModal(parseInt(oversBtn.dataset.viewOversPoId, 10), oversBtn.dataset.viewOversPoNumber || ''), 100);
        return;
    }

    const receiveOverlay = event.target.closest('[data-close-receive-details-overlay]');
    if (receiveOverlay && event.target === receiveOverlay) {
        closeReceiveDetailsModal(event);
        return;
    }
    if (event.target.closest('[data-close-receive-details-modal]')) {
