...
- **Writes:** `append_workflow_event` (events) and `try_finalize` / `create_workflow_bag_with_card` / `assign_inventory_bag_to_card` (staff: link receiving `bags` row → workflow + card; staff UI resolves `bags.id` by product/tablet flavor + box # + bag # via `workflow_bag_lookup`, with disambiguation when multiple receives match) / `force_release_card` (policy plus `qr_cards` mutex). Terminal policy lives in `workflow_finalize.py`, not the read layer. **Packaging sync:** on `PACKAGING_SNAPSHOT`, `workflow_warehouse_bridge.upsert_packaged_from_workflow_packaging` replaces the single `warehouse_submissions` row with `receipt_number = WORKFLOW-<workflow_bag_id>` (same tablet math as the packaged form) so Submissions / PO bag totals include QR workflow packaging.
