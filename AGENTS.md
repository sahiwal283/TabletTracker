...
- **Writes:** `append_workflow_event` (events) and `try_finalize` / `create_workflow_bag_with_card` / `force_release_card` (policy plus `qr_cards` mutex). Terminal policy lives in `workflow_finalize.py`, not the read layer.
