...
- **Admin** `/admin/workflow-qr`: lists stations and QR cards; **Release** calls `force_release_card` (undo card↔bag assignment for testing). `workflow_stations.machine_id` → `machines.id` (sealing role) links floor station to production machine count machines; **Save** on that page updates the mapping. **Add sealing station** POSTs to `workflow_qr_add_station` (optional scan token, else auto `seal-…` hex).
